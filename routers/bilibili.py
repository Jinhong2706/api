...
async def _request(client: httpx.AsyncClient, url: str, params: dict = None) -> dict:
    resp = await client.get(url, params=params, headers=BILIBILI_HEADERS)
    return resp.json()
...
@router.get("/hot")
async def get_hot(ps: int = Query(50, le=100)):
...
async def get_video_download(bvid: str):
    ...
    cid = info.get("data", {}).get("cid")
    return await _get_play_url(client, {"bvid": bvid, "cid": cid}, qn=80, fnval=0)
...
async def get_video_download_1080(bvid: str):
    ...
    info = await _get_video_info(client, bvid=bvid)
    cid = info.get("data", {}).get("cid")
    return await _get_play_url(client, {"bvid": bvid, "cid": cid}, qn=80, fnval=16)
...
async def get_video_download_by_aid(aid: int):
    client = await get_http_client()
    info = await _get_video_info(client, aid=aid)
    cid = info.get("data", {}).get("cid")
    bvid = info.get("data", {}).get("bvid")
    return await _get_play_url(client, {"bvid": bvid, "cid": cid}, qn=80, fnval=0)

@router.post("/video/download/aid")
async def post_video_download_by_aid(aid: int):
    client = await get_http_client()
    info = await _get_video_info(client, aid=aid)
    cid = info.get("data", {}).get("cid")
    bvid = info.get("data", {}).get("bvid")
...
async def get_video_download_aid_1080(aid: int):
    client = await get_http_client()
    info = await _get_video_info(client, aid=aid)
    cid = info.get("data", {}).get("cid")
    bvid = info.get("data", {}).get("bvid")
...
async def post_video_download_aid_1080(aid: int):
    client = await get_http_client()
    info = await _get_video_info(client, aid=aid)
    cid = info.get("data", {}).get("cid")
    bvid = info.get("data", {}).get("bvid")
